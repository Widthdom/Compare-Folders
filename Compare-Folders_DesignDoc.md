...
| `.ps1`, `.bat`, `.md`, `.txt`, `.yml`    | テキスト比較                       | プレーンテキストとしてファイル内容を比較                     |
...
渡されたILコード同士を比較し、差分をマークダウンのdiff形式で返す。

- 引数:
  - `$lines1`: 比較対象ILコード1
  - `$lines2`: 比較対象ILコード2

- 戻り値: 渡されたILコード同士の差分（マークダウンのdiff形式）
...
- 比較方法は以下の通り自動判別される：
  ...
  - `.dll`, `.exe`: `dotnet-ildasm` を使って ILコードを比較（`^// MVID:`を除く）
  - その他: MD5ハッシュで比較
...
- `-ShowDetailedDiff`  
  （ブーリアン）true の場合、`.dll` や `.exe`のILコード差分をレポートに出力する。
